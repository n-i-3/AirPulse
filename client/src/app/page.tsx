...
const ModernCard = ({ title, children, className, icon: Icon, trend }: { title: string, children: React.ReactNode, className?: string, icon?: any, trend?: string }) => (
  <div className={cn(
    "group relative flex flex-col gap-3 rounded-2xl border border-white/5 bg-card backdrop-blur-xl p-5 transition-all duration-300 hover:border-white/10 hover:shadow-2xl hover:shadow-primary/5",
    className
  )}>
    {/* Header */}
    <div className="flex items-center justify-between">
      ...
      {trend && <span className="rounded-full bg-green-500/10 px-2 py-0.5 text-[10px] font-bold text-green-500 font-mono">{trend}</span>}
    </div>
    ...
    {/* Shine Effect */}
    <div className="absolute inset-0 -z-10 bg-gradient-to-br from-white/5 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 rounded-2xl" />
  </div>
);
    ...
    <main className="min-h-screen bg-transparent text-foreground font-sans">
      ...
      <div className="mx-auto max-w-screen-2xl px-4 py-2 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <ModernCard title="Avg AQI (Delhi)" icon={Wind} trend="Live">
            <div className="flex items-end gap-3 mt-1">
              {loading ? (
                <span className="text-4xl font-bold tracking-tighter text-white animate-pulse">---</span>
              ) : (
                <>
                  <span className="text-4xl font-bold tracking-tighter text-white">{avgAqi}</span>
          ...
          <ModernCard title="Active Reporters" icon={Users} trend="+14 Active">
            <div className="flex items-end gap-3 mt-1">
              <span className="text-4xl font-bold tracking-tighter text-white">1,203</span>
          ...
          <ModernCard title="Critical Zones" icon={AlertTriangle} className={criticalZones.length > 0 ? "border-red-500/20 bg-red-500/5" : ""}>
            <div className="flex items-end gap-3 mt-1">
              {loading ? (
                <span className="text-4xl font-bold tracking-tighter text-white animate-pulse">--</span>
              ) : (
                <>
                  <span className="text-4xl font-bold tracking-tighter text-white">{criticalZones.length.toString().padStart(2, '0')}</span>
                  <span className="mb-1 text-sm text-red-400 font-medium">Require Mitigation</span>
                </>
              )}
            </div>
            <div className="mt-4 flex flex-wrap gap-2 text-[10px] font-mono uppercase">
              {loading ? (
                <span className="px-2 py-1 rounded bg-white/5 text-zinc-500">Loading...</span>
              ) : criticalZones.length > 0 ? (
                criticalZones.map((zone, i) => (
                  <span key={i} className="px-2 py-1 rounded bg-red-500/20 text-red-200 border border-red-500/20">{zone}</span>
                ))
              ) : (
                <span className="px-2 py-1 rounded bg-green-500/20 text-green-200 border border-green-500/20">All Clear</span>
              )}
            </div>
          </ModernCard>
          ...
          <ModernCard title="AI Confidence" icon={Activity} trend="High">
            <div className="flex items-end gap-3 mt-1">
              <span className="text-4xl font-bold tracking-tighter text-white">89.4%</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[700px]">
          ...
          {/* Map Module */}
          <div className="lg:col-span-8 h-full rounded-3xl border border-white/5 bg-card/50 backdrop-blur-xl overflow-hidden relative shadow-2xl">
            {/* Map Overlay Stats */}
            <div className="absolute top-4 left-4 z-[400] flex flex-col gap-2 pointer-events-none">
              <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-lg px-3 py-2 text-white shadow-lg">
                <span className="text-[10px] text-zinc-400 uppercase font-bold block">Live Feed</span>
          ...
          <div className="lg:col-span-4 flex flex-col gap-4 h-full">
            ...
            <div className="flex-1 rounded-3xl border border-primary/20 bg-gradient-to-b from-primary/10 to-transparent p-6 relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10">
                {loading ? (
                  <h4 className="text-2xl font-medium leading-tight text-white animate-pulse">
                ...
                <div className="mt-4 p-4 rounded-xl bg-black/40 border border-white/5 backdrop-blur-sm">
                  <div className="flex justify-between items-center text-xs font-mono mb-2 text-zinc-500 uppercase">
            ...
            <div className="flex-1 rounded-3xl border border-white/5 bg-card/30 backdrop-blur-xl p-6">
              <h3 className="font-bold tracking-tight mb-4 flex items-center justify-between">
                <span>Verified Reports</span>
                <span className="text-xs font-normal text-muted-foreground bg-white/5 px-2 py-1 rounded-full">Real-time</span>
              </h3>

              <div className="space-y-3 overflow-y-auto max-h-[250px] pr-2 scrollbar-none">
                {[1, 2, 3].map((_, i) => (
                  <div key={i} className="group flex gap-4 items-start p-3 rounded-2xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/5 cursor-pointer">
                    <div className="mt-1 h-2 w-2 rounded-full bg-primary shadow-[0_0_8px_var(--color-primary)]" />
                    <div>
                      <h5 className="text-sm font-medium text-white group-hover:text-primary transition-colors">Illegal Waste Burning</h5>
                      <p className="text-xs text-zinc-400 mt-0.5">Reported 2m ago from Punjabi Bagh</p>
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-[10px] font-mono bg-white/5 px-1.5 py-0.5 rounded text-zinc-500">Hash: 0x82...9f</span>
